<?php
	//XMLEntities
	$g_XMLEntities = array();
	$g_XMLEntities[] = new PS_CSV_XMLEntity(0x26, "&amp;");
	$g_XMLEntities[] = new PS_CSV_XMLEntity(0x22, "&quot;");
	$g_XMLEntities[] = new PS_CSV_XMLEntity(0x27, "&apos;");
	$g_XMLEntities[] = new PS_CSV_XMLEntity(0x3c, "&lt;");
	$g_XMLEntities[] = new PS_CSV_XMLEntity(0x3e, "&gt;");

	abstract class PS_CSV_Nullable
	{
	...
	class PS_CSV_NullableInt extends PS_CSV_Nullable
	{
		private $m_nValue;
	...
	class PS_CSV_NullableBool extends PS_CSV_Nullable
	{
		private $m_boValue;
	...
	class PS_CSV_ISOCountryList
	{
		...
		public function getISOCountry($szCountryShort, &$icISOCountry)
		{
					...
					$nCount < count($this->m_licISOCountries))
			{
			    ...
			    {
			    	$icISOCountry = new PS_CSV_ISOCountry($icISOCountry2->getISOCode(), $icISOCountry2->getCountryName(), $icISOCountry2->getCountryShort(), $icISOCountry2->getListPriority());
			    	$boFound = true;
			    }
		...
		{
			$newISOCountry = new PS_CSV_ISOCountry($nISOCode, $szCountryName, $szCountryShort, $nListPriority);
	...
	class PS_CSV_ISOCurrencyList
	{
		...
		public function getISOCurrency($szCurrencyShort, &$icISOCurrency)
		{
	        ...
	        while (!$boFound &&
	              	$nCount < count($this->m_licISOCurrencies))
	     	{
	            ...
	            {
	            	$icISOCurrency = new PS_CSV_ISOCurrency($icISOCurrency2->getISOCode(), $icISOCurrency2->getCurrency(),$icISOCurrency2->getCurrencyShort(), $icISOCurrency2->getExponent());
	                $boFound = true;
	          	}
		...
		{
			$newISOCurrency = new PS_CSV_ISOCurrency($nISOCode, $szCurrency, $szCurrencyShort, $nExponent);
	...
	class PS_CSV_SharedFunctions
	{
		public static function getNamedTagInTagList($szName, $xtlTagList)
		{
	        ...
	        $lszHierarchicalNames = PS_CSV_SharedFunctions::getStringListFromCharSeparatedString($szName, '.');
		...
		public static function getStringListFromCharSeparatedString($szString, $cDelimiter)
		{
	        ...
	        if ($szString == null ||
	      	...
	      	$lszStringList = new PS_CSV_StringList();
		...
		public static function getValue($szXMLVariable, $xtlTagList, & $szValue)
		{
			...
			if (xtlTagList == null)
	       	...
	       	$lszHierarchicalNames = new PS_CSV_StringList();
	        $szValue = null;
	        $lszHierarchicalNames = PS_CSV_SharedFunctions::getStringListFromCharSeparatedString($szXMLVariable, '.');
	        
			if (count($lszHierarchicalNames) == 1)
	     	{
	       		$szXMLTagName = $lszHierarchicalNames->getAt(0);

	            $xtCurrentTag = PS_CSV_SharedFunctions::GetNamedTagInTagList($szXMLTagName, $xtlTagList);
	    		...
	    		if (count($lszHierarchicalNames) > 1)
	          	{
	                ...
	                if ($xtCurrentTag != null)
	                {
	                    ...
	                    else
	                    {
	                        ...
	                        $xtCurrentTag = PS_CSV_SharedFunctions::getNamedTagInTagList($szLastXMLTagName, $xtCurrentTag->getChildTags());

	                        if ($xtCurrentTag != null)
	                        {
	                        	$szValue = PS_CSV_SharedFunctions::replaceEntitiesInStringWithChars($xtCurrentTag->getContent());
	                            $boReturnValue = true;
	                      	}
	                   	}
	             	}
