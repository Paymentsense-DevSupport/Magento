<?php
	...
	class PS_CSV_SOAPNamespaceList
	{
		...
		private function add1(PS_CSV_SOAPNamespace $snSOAPNamespace)
		{
		...
		private function add2($szPrefix, $szSOAPNamespace)
		{
			...
			$this->m_lsnSOAPNamespaceList[] = new PS_CSV_SOAPNamespace($szPrefix, $szSOAPNamespace);
		}
	...
	class PS_CSV_SOAPParameter
	{
		private $m_szName;
	 	...
	 	private $m_lspaSOAPParamAttributeList;
	   	...
	   	{
			$this->m_szValue = PS_CSV_SharedFunctions::replaceCharsInStringWithEntities($szValue);
	   	}
	   	...
	   	//constructor
	   	public function __construct($szName, $szValue, PS_CSV_SOAPParamAttributeList $lspaSOAPParamAttributeList = null)
	   	{
	   		$nCount = 0;
	   		...
	   		$this->m_lspSOAPParamList = new PS_CSV_SOAPParamList();
			$this->m_lspaSOAPParamAttributeList = new PS_CSV_SOAPParamAttributeList();
	   		
	   		if ($lspaSOAPParamAttributeList != null)
	   		{
	   			for ($nCount = 0; $nCount < $lspaSOAPParamAttributeList->getCount();$nCount++)
	   			{
	   				$spaSOAPParamAttribute = new PS_CSV_SOAPParamAttribute($lspaSOAPParamAttributeList->getAt($nCount)->getName(), $lspaSOAPParamAttributeList->getAt($nCount)->getValue());
	   	...
	   	function toXMLString()
	   	{
	   		...
	   		if ($this->m_lspaSOAPParamAttributeList != null)
	   		{
	   			for ($nCount = 0; $nCount < $this->m_lspaSOAPParamAttributeList->getCount(); $nCount++)
	   			{
		   			...
		   			{
		   				$sbString = '';
		   				$sbString .= ' ' .$spaAttribute->getName(). '="' .PS_CSV_SharedFunctions::replaceCharsInStringWithEntities($spaAttribute->getValue()). '"';
	   		...
	   		else
	   		{
	   			...
	   			if ($this->getValue() != '')
	   			{
	   				$sbReturnString .= PS_CSV_SharedFunctions::replaceCharsInStringWithEntities($this->getValue());
	   			}
	...
	class PS_CSV_SOAPParamList
	{
		...
		protected function add1(PS_CSV_SOAPParameter $spSOAPParam)
		{
		...
		protected function add2($szName, $szValue)
		{
			...
			if ($szName != '' &&
				$szName != null)
			{
				$this->m_lspSOAPParamList[] = new PS_CSV_SOAPParameter($szName, $szValue);
			}
	...
	class PS_CSV_SOAPParamAttribute
	{
		private $m_szName;
	   	private $m_szValue;
	...
	class PS_CSV_SOAPParamAttributeList
	{
		...
		private function add2($szName, $szValue)
		{
			...
			if ($szName != '' &&
				$szName != null)
			{
				$nReturnValue = array_push($this->m_lspaSOAPParamAttributeAttributeList, new PS_CSV_SOAPParamAttribute($szName, $szValue));
			}
	...
	class PS_CSV_SOAP
	{
	    ...
	    public function addParam1($szName, $szValue, PS_CSV_SOAPParamAttributeList $lspaSOAPParamAttributeList = null)
	    {
	    	$spSOAPParam;
	    	
	    	$spSOAPParam = new PS_CSV_SOAPParameter($szName, $szValue, $lspaSOAPParamAttributeList);
	    	
	    	$this->addParam2($spSOAPParam, true);
	    }
	    public function addParam2(PS_CSV_SOAPParameter $spSOAPParam, $boOverWriteValue)
	    {
	        ...
	        $lszHierarchicalNames = new PS_CSV_StringList();
	        
	        $lszHierarchicalNames = PS_CSV_SharedFunctions::getStringListFromCharSeparatedString($spSOAPParam->getName(), '.');
	        ...
	        else 
	        {
	        	...
	        	for ($nCount = 0; $nCount < $lszHierarchicalNames->getCount(); $nCount++)
	        	{
	        		// get the current tag name
	              	...
	              	//continuework
	               	$szTagNameToFind = PS_CSV_SharedFunctions::getArrayNameAndIndex($szString, $nCurrentIndex);
	             		...
	             		$nCount == 0)
	             	{
	                    ...
	                    $spWorkingSOAPParam = PS_CSV_Functions::isSOAPParamInParamList($lspParamList, $szTagNameToFind, $nCurrentIndex);
	                        ...
	                        if ($nCount == ($lszHierarchicalNames->getCount() - 1))
	                        {
	                            ...
	                            for ($nCount2 = 0; $nCount2 < $spSOAPParam->getSOAPParamAttributeList()->getCount(); $nCount2++)
	                            {
	                                ...
	                                $spaAttribute = $spSOAPParam->getSOAPParamAttributeList()->getAt($nCount2);
	                              	...
	                              	if ($spaAttribute != null)
	                                {
	                                	$spaNewAttribute = new PS_CSV_SOAPParamAttribute($spaAttribute->getName(), $spaAttribute->getValue());
	             	...
	             	if (!$boFound)
	                {
	                    ...
	                    if ($nCount == ($lszHierarchicalNames->getCount() - 1))
	                    {
	                    	$lspaAttributeList = new PS_CSV_SOAPParamAttributeList();
	                        ...
	                        $spNewSOAPParam = new PS_CSV_SOAPParameter($szTagNameToFind, $spSOAPParam->getValue(), $lspaAttributeList);

	                        $lspParamList->add($spNewSOAPParam);
	                 	}
	                    else
	                    {
	                    	$spNewSOAPParam = new PS_CSV_SOAPParameter($szTagNameToFind, '', null);
	                        $lspParamList->add($spNewSOAPParam);
	    ...
	    private function addParamAttribute1($szName, $szParamAttributeName, $szParamAttributeValue)
	    {
	    	...
	    	$lspaSOAPParamAttributeList = new PS_CSV_SOAPParamAttributeList();
	    	$spaSOAPParamAttribute = new PS_CSV_SOAPParamAttribute($szParamAttributeName, $szParamAttributeValue);
	    	$lspaSOAPParamAttributeList->add($spaSOAPParamAttribute);
	    	
	    	$spSOAPParam = new PS_CSV_SOAPParameter($szName, '', $lspaSOAPParamAttributeList);
	    	
	    	$this->addParam2($spSOAPParam, false);
	    }
	    private function addParamAttribute2($szName, PS_CSV_SOAPParamAttribute $spaSOAPParamAttribute)
	    {
	    	$spSOAPParam;
	    	$lspaSOAPParamAttributeList;
	    ...
	    private function SOAP3($szMethod, $szMethodURI, $szURL, $szSOAPEncoding, $boAddDefaultNamespaces, PS_CSV_SOAPNamespaceList $lsnSOAPNamespaceList = null)
	    {
	    	$snSOAPNamespace;
	       	$nCount = 0;
	        ...
	        $this->m_lsnSOAPNamespaceList = new PS_CSV_SOAPNamespaceList();

	      	if ($boAddDefaultNamespaces)
	        {
	        	$snSOAPNamespace = new PS_CSV_SOAPNamespace('soap', 'http://schemas.xmlsoap.org/soap/envelope/');
	          	$this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	            $snSOAPNamespace = new PS_CSV_SOAPNamespace('xsi', 'http://www.w3.org/2001/XMLSchema-instance');
	            $this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	           	$snSOAPNamespace = new PS_CSV_SOAPNamespace('xsd', 'http://www.w3.org/2001/XMLSchema');
	            $this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	        }
	        if ($lsnSOAPNamespaceList != null)
	      	{
	            ...
	            {
	             	$snSOAPNamespace = new PS_CSV_SOAPNamespace($lsnSOAPNamespaceList->getAt($nCount)->getPrefix(), $lsnSOAPNamespaceList->getAt($nCount)->getNamespace());
	              	$this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	            }
	        }
	        $this->m_lspSOAPParamList = new PS_CSV_SOAPParamList();
	...
	class PS_CSV_Functions
	{
		public static function isSOAPParamInParamList(PS_CSV_SOAPParamList $lspParamList, $szTagNameToFind, $nIndex)
		{
			$spReturnParam = null;
			$boFound = false;
			$nFound = 0;
