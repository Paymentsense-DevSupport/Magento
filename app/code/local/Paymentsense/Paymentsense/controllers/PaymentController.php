...
if (!defined('COMPILER_INCLUDE_PATH')) {
	require_once "app/code/local/Paymentsense/Paymentsense/Model/Common/PaymentFormHelper.php";
} else {
 ...
 *
 */
class Paymentsense_Paymentsense_PaymentController extends Mage_Core_Controller_Front_Action
{
    ...
    {
    	$this->getResponse()->setBody($this->getLayout()->createBlock('Paymentsense/redirect')->toHtml());
    }
    ...
    {
    	$this->getResponse()->setBody($this->getLayout()->createBlock('Paymentsense/threedsecure')->toHtml());
    }
    ...
    public function callback3dAction()
    {
    	...
    	try
    	{
    		...
    		$PaymentsenseOrderId = Mage::getSingleton('checkout/session')->getPaymentsenseOrderId();
    	...
    	catch (Exception $exc)
    	{
    		...
    		if( isset($_SESSION['Paymentsense_message']) )
    		{
    			$szMessage = $_SESSION['Paymentsense_message'];
    			unset($_SESSION['Paymentsense_message']);
    		}
    		else
    		{
				$szMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_7655;
    		}
    	}
		...
		if ($boError)
    	{
			...
			if($order)
			{
				...
				$order->setCustomerNote(Mage::helper('Paymentsense')->__('3D Secure Authentication Failed'));
    ...
    public function callbackhostedpaymentAction()
    {
    	...
    	try
    	{
    		...
    		if(!PS_CSV_PaymentFormHelper::compareHostedPaymentFormHashDigest($formVariables, $szPassword, $hmHashMethod, $szPreSharedKey))
    		{
    			$boError = true;
    	...
    	{
    		$boError = true;
    		$szNotificationMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_183;
    		Mage::logException($exc);
    	}
    	...
    	if($boError)
    	{
			...
			if($order)
			{
				...
				$order->setCustomerNote(Mage::helper('Paymentsense')->__('Hosted Payment Failed'));
				$order->setState($orderState, $orderStatus, $szPaymentProcessorResponse, false);
				$order->save();
			}
    ...
    public function serverpullresultAction()
    {
    	...
    	{
    		$szErrorMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_309;
    		Mage::log(Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_309." Request URI: ".$szURLVariableString);
    	}
    	else
    	{
	    	$szURLVariableString = substr($szURLVariableString, $nStartIndex + 1);
	    	$arFormVariables = PS_CSV_PaymentFormHelper::getVariableCollectionFromString($szURLVariableString);
	    	
	    	if(!PS_CSV_PaymentFormHelper::compareServerHashDigest($arFormVariables, $szPassword, $hmHashMethod, $szPreSharedKey))
	   		{
	   			// report an error message
	   			$szErrorMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_304;
	   		}
	   		else
	   		{
				...
				$szErrorMessage = curl_error($cCurl);
	   				...
	   				$nErrorNumber > 0)
	   			{
	   				...
	   				$szErrorMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_329." Message: ".$szErrorMessage;
	   			}
	   			else
	   			{
	   				...
	   				try
	   				{
		   				...
		   				$responseItems = PS_CSV_PaymentFormHelper::getVariableCollectionFromString($response);
		   				...
		   				if($szStatusCode !== '0')
		   				{
		   					$szErrorMessage = Paymentsense_Paymentsense_Model_Common_GlobalErrors::ERROR_381;
		   				...
		   				else
		   				{
		   					...
		   					$transactionResult = PS_CSV_PaymentFormHelper::getVariableCollectionFromString($transactionResult);
    ...
    private function _threeDSecureAuthenticationRequired($szPassword, $hmHashMethod, $szPreSharedKey)
    {
    	...
    	if(!PS_CSV_PaymentFormHelper::compareThreeDSecureAuthenticationRequiredHashDigest($formVariables, $szPassword, $hmHashMethod, $szPreSharedKey))
    	{
    	...
    	else
    	{
    		...
    		Mage::getSingleton('checkout/session')->setMd($formVariables['CrossReference'])
	        										->setAcsurl($formVariables['ACSURL'])
			  		   								...
			  		   								->setTermurl('Paymentsense/payment/callbacktransparentredirect');
			
			// redirect to a 3D Secure page
			$this->_redirect('Paymentsense/payment/threedsecure');
    	}
    }
    ...
    private function _postThreeDSecureAuthentication($szPassword, $hmHashMethod, $szPreSharedKey)
    {
    	...
    	$szCallbackURL = Mage::getUrl('Paymentsense/payment/callbacktransparentredirect', array('_secure' => true));
    	$szHashDigest = PS_CSV_PaymentFormHelper::calculatePostThreeDSecureAuthenticationHashDigest($szMerchantID, $szPassword, $hmHashMethod, $szPreSharedKey, $szPaRES, $szCrossReference, $szTransactionDateTime, $szCallbackURL);
    	...
    	$this->_redirect('Paymentsense/payment/redirect');
    }
    ...
    private function _paymentComplete($szPassword, $hmHashMethod, $szPreSharedKey)
    {
    	...
    	try
    	{
		    ...
		    if(!PS_CSV_PaymentFormHelper::comparePaymentCompleteHashDigest($formVariables, $szPassword, $hmHashMethod, $szPreSharedKey))
	    	{
	    		$boError = true;
	    		$szNotificationMessage = "The payment was rejected for a SECURITY reason: the incoming payment data was tampered with.";
    	...
    	if($boError == true)
    	{
			...
			if($order)
			{
				$orderState = 'pending_payment';
				$orderStatus = 'csv_failed_hosted_payment';
				$order->setCustomerNote(Mage::helper('Paymentsense')->__('Transparent Redirect Payment Failed'));
				$order->setState($orderState, $orderStatus, $szPaymentProcessorResponse, false);
			}
    ...
    private function _fixBackButtonBug($szOrderID, $szStatusCode, $szMessage, $szPreviousStatusCode, $szPreviousMessage)
    {
    	...
    	$mode = Mage::getModel('Paymentsense/direct')->getConfigData('mode');
    ...
    {
    	$model = Mage::getSingleton('Paymentsense/direct');
    	...
    	$result = Mage::getModel('Paymentsense/direct')->csvVoid($payment);
    ...
    public function collectionAction()
    {
    	...
    	$result = Mage::getModel('Paymentsense/direct')->csvCollection($payment, $szOrderID, $szCrossReference);
